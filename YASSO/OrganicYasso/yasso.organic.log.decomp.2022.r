#modified for calculation of litter decomposition from natural mortality and logging residues in organic forest soils by AW
#22.9.2022 /AW
################## NOTE HERE USING CONSTANT CLIMATE
# Using scand. parameters for reporting
### THE LAST YEAR OF CALCULATION 2012
####modified for NEW weather data from VMI plots 15.3.2022 AW
###modified to exclude roots, foliage and branches from natural mortality 30.8.2022 AW
yr <- 2019
yr.VANHA <- 2018
...
resultspath <- paste(path,'/development/organicsoils/OrganicYasso/', sep="")
#crf <- paste(path,yr,'/crf/', sep="")
#NIR <- paste(path,yr,'/NIR/',sep="")


source("YASSO/OrganicYasso/y07_subroutine.r")
...
climate =  read.table(paste(climpath,"GHG_climate_1960_2019_new.csv",sep=""), header = TRUE, sep=",") 
#using climate data calculated based on location of VMI12 sample plots
...
#tree and vegetation litter removed
...
#above-ground and below-ground litter from logging residues
...
#summing similar litter inputs
log.nwl.SF.org <- log.nwl.SF.org.abv+log.nwl.SF.org.bel
log.fwl.SF.org <- log.fwl.SF.org.abv+log.fwl.SF.org.bel
log.cwl.SF.org <- log.cwl.SF.org.abv
...
#above-ground and below-ground litter from natural mortality
...
#summing similar litter inputs
nat.nwl.SF.org <- nat.nwl.SF.org.abv+nat.nwl.SF.org.bel
nat.fwl.SF.org <- nat.fwl.SF.org.abv+nat.fwl.SF.org.bel
nat.cwl.SF.org <- nat.cwl.SF.org.abv

#log1 <- log.nwl.SF.org.abv[3:dim(log.nwl.SF.org.abv)[1], ]
#nat1 <-nat.nwl.SF.org.abv[3:dim(nat.nwl.SF.org.abv)[1], ]
...
log.nwl.SF.org.abv.spin <- colMeans(log.nwl.SF.org.abv[1:7,])
log.nwl.SF.org.bel.spin <- colMeans(log.nwl.SF.org.bel[1:7,])
log.fwl.SF.org.abv.spin <- colMeans(log.fwl.SF.org.abv[1:7,])
log.fwl.SF.org.bel.spin <- colMeans(log.fwl.SF.org.bel[1:7,])
log.cwl.SF.org.abv.spin <- colMeans(log.cwl.SF.org.abv[1:7,])

nat.nwl.SF.org.abv.spin <- colMeans(nat.nwl.SF.org.abv[1:7,])
nat.nwl.SF.org.bel.spin <- colMeans(nat.nwl.SF.org.bel[1:7,])
nat.fwl.SF.org.abv.spin <- colMeans(nat.fwl.SF.org.abv[1:7,])
nat.fwl.SF.org.bel.spin <- colMeans(nat.fwl.SF.org.bel[1:7,])
nat.cwl.SF.org.abv.spin <- colMeans(nat.cwl.SF.org.abv[1:7,])

#  sum(nat.nwl.SF.org.abv.spin,nat.nwl.SF.org.bel.spin, nat.fwl.SF.org.abv.spin,nat.fwl.SF.org.bel.spin, nat.cwl.SF.org.abv.spin)
#  sum(log.nwl.SF.org.abv.spin,log.nwl.SF.org.bel.spin, log.fwl.SF.org.abv.spin,log.fwl.SF.org.bel.spin, log.cwl.SF.org.abv.spin)
###    
...
nwl <- log.nwl.SF.org.abv.spin+log.nwl.SF.org.bel.spin+nat.nwl.SF.org.abv.spin+nat.fwl.SF.org.bel.spin 
fwl <- log.fwl.SF.org.abv.spin+log.fwl.SF.org.bel.spin+nat.fwl.SF.org.abv.spin+nat.fwl.SF.org.bel.spin 
cwl <- log.cwl.SF.org.abv.spin+nat.cwl.SF.org.abv.spin
cwl
nwl.spin.sf=c(nwl,0)
fwl.spin.sf=c(fwl,0)
cwl.spin.sf=c(cwl,0)
...
##    sum(fwl, cwl, nwl)   gives  2.47169 tons C /ha (reasonable)
   ##    sum(fwl, cwl, nwl)   for  log+nat gives  2.47169 tons C /ha (reasonable)
##  ACTUAL SPIN-UP SOUTH!!


##  ACTUAL SPIN-UP SOUTH!!

#Time for spin-up in years
t <- 50 #time in years 
...
spin.nwl.sf.s <- yasso07(MeanTemperature = cl[1], 
                         Precipitation = cl[2], 
                         TemperatureAmplitude = cl[3], 
                         InitialCPool = init, 
                         LitterInput = inf, 
                         WoodySize = s,
                         Yasso07Parameters = a.ska,
                         SimulationTime = t)
...
spin.fwl.sf.s <- yasso07(MeanTemperature = cl[1], 
        Precipitation = cl[2], 
        TemperatureAmplitude = cl[3], 
        InitialCPool = init, 
        LitterInput = inf, 
...
spin.cwl.sf.s <- yasso07(MeanTemperature = cl[1], 
                        Precipitation = cl[2], 
                        TemperatureAmplitude = cl[3], 
                        InitialCPool = init, 
                        LitterInput = inf, 
...
write.csv(spin.sf.sum, "new.spin.sf.sum.csv")
...
#summing similar litter inputs
log.nwl.NF.org <- log.nwl.NF.org.abv+log.nwl.NF.org.bel
log.fwl.NF.org <- log.fwl.NF.org.abv+log.fwl.NF.org.bel
log.cwl.NF.org <- log.cwl.NF.org.abv

#summing similar litter inputs
nat.nwl.NF.org <- nat.nwl.NF.org.abv+nat.nwl.NF.org.bel
nat.fwl.NF.org <- nat.fwl.NF.org.abv+nat.fwl.NF.org.bel
nat.cwl.NF.org <- nat.cwl.NF.org.abv

# for loggings and natural mortality NF spin-up is an average of 1982-85 corrected by AW?
...
log.nwl.NF.org.abv.spin <- colMeans(log.nwl.NF.org.abv[1:3,])
log.nwl.NF.org.bel.spin <- colMeans(log.nwl.NF.org.bel[1:3,])
log.fwl.NF.org.abv.spin <- colMeans(log.fwl.NF.org.abv[1:3,])
log.fwl.NF.org.bel.spin <- colMeans(log.fwl.NF.org.bel[1:3,])
log.cwl.NF.org.abv.spin <- colMeans(log.cwl.NF.org.abv[1:3,])

nat.nwl.NF.org.abv.spin <- colMeans(nat.nwl.NF.org.abv[1:3,])
nat.nwl.NF.org.bel.spin <- colMeans(nat.nwl.NF.org.bel[1:3,])
nat.fwl.NF.org.abv.spin <- colMeans(nat.fwl.NF.org.abv[1:3,])
nat.fwl.NF.org.bel.spin <- colMeans(nat.fwl.NF.org.bel[1:3,])
nat.cwl.NF.org.abv.spin <- colMeans(nat.cwl.NF.org.abv[1:3,])
...
#sums for natural mortality and logging residues
#  sum(nat.nwl.NF.org.abv.spin,nat.nwl.NF.org.bel.spin, nat.fwl.NF.org.abv.spin,nat.fwl.NF.org.bel.spin, nat.cwl.NF.org.abv.spin)
#  sum(log.nwl.NF.org.abv.spin,log.nwl.NF.org.bel.spin, log.fwl.NF.org.abv.spin,log.fwl.NF.org.bel.spin, log.cwl.NF.org.abv.spin)
...
nwl <- log.nwl.NF.org.abv.spin+log.nwl.NF.org.bel.spin+nat.nwl.NF.org.abv.spin+nat.fwl.NF.org.bel.spin 
fwl <- log.fwl.NF.org.abv.spin+log.fwl.NF.org.bel.spin+nat.fwl.NF.org.abv.spin+nat.fwl.NF.org.bel.spin 
cwl <- log.cwl.NF.org.abv.spin+nat.cwl.NF.org.abv.spin
...
##    sum(fwl, cwl, nwl) for log+nat   gives   7.0  tons C /ha (reasonable?)
...
spin.nwl.nf.s <- yasso07(MeanTemperature = cl[1], 
        Precipitation = cl[2], 
        TemperatureAmplitude = cl[3], 
        InitialCPool = init, 
        LitterInput = inf, 
...
spin.fwl.nf.s <- yasso07(MeanTemperature = cl[1], 
           Precipitation = cl[2], 
           TemperatureAmplitude = cl[3], 
           InitialCPool = init, 
           LitterInput = inf, 
...
spin.cwl.nf.s <- yasso07(MeanTemperature = cl[1], 
                         Precipitation = cl[2], 
                         TemperatureAmplitude = cl[3], 
                         InitialCPool = init, 
                         LitterInput = inf, 
...
write.csv(spin.nf.sum, "new.spin.nf.sum.csv")

## printing C inputs for the LUC-calculation ######
## spin up start values= loggings and natural mortality SF spin-up is an average of 1970-1976###
...
#write.table(FL.spin.litter, paste(resultspath,"FL.soil.init.CInput.csv", sep=""), sep=" ", row.names = TRUE, col.names = TRUE, quote=FALSE)
write.table(FL.spin.litter, "NEW.FL.soil.init.CInput.csv", sep=" ", row.names = TRUE, col.names = TRUE, quote=FALSE)
...
###only log and nat for starting year 1972- ######
#here removing roots, foliage and branched from natural mortality
#nwl <- log.nwl.SF.org[3:dim(log.nwl.SF.org)[1], ]+nat.nwl.SF.org[3:dim(nat.nwl.SF.org)[1], ]
#fwl <- log.fwl.SF.org[3:dim(log.fwl.SF.org)[1], ]+nat.fwl.SF.org[3:dim(nat.fwl.SF.org)[1], ]
nwl <- log.nwl.SF.org[3:dim(log.nwl.SF.org)[1], ]
fwl <- log.fwl.SF.org[3:dim(log.fwl.SF.org)[1], ]
cwl <- log.cwl.SF.org[3:dim(log.cwl.SF.org)[1], ]+nat.cwl.SF.org[3:dim(nat.cwl.SF.org)[1], ]
...
write.csv(litter.sf, (paste(resultspath,"corrected.litter.sf.",yr,".csv", sep="")))
litter.sf
litter.sf.awenh <- cbind(nwl, fwl, cwl)
#write.csv(litter.sf.awenh, (paste(resultspath,"litter.sf.awenh.csv", sep="")))
write.csv(litter.sf.awenh, (paste(resultspath,"corrected.litter.sf.awenh.csv", sep="")))

##calculating total litter from 1972 onwards
sum.litter.sf <- cbind(rowSums(litter.sf))
sum.litter.sf
write.csv(sum.litter.sf, (paste(resultspath,"sum.litter.sf.csv", sep="")))

##starting total litter from 1973 by deleting the first row for calculating decomposition later in the script as a difference between yasso result and total litter


littersum.sf <- cbind(sum.litter.sf[-1,])
littersum.sf
...
write.csv(soil.sf.awenh, (paste(resultspath,"corrected.soil.sf.awenh.csv", sep="")))
...
#calculating decomposition of logging residues as a difference between litter input and yasso results from logging residues, t C/ha
#starting from 1973

difference.sf  <-cbind(d.soil.c.sf.s)
difference.sf

log.decomp.sf <- littersum.sf-difference.sf
log.decomp.sf

write.csv(log.decomp.sf, (paste(resultspath,"log.decomp.sf.csv", sep="")))
...
write.csv(d.soil.c.sf.s.out, (paste(resultspath,"corrected.d.soil.c.tot.sf.s.mean.csv", sep="")))
...
#here removing roots, foliage and branches from natural mortality
#nwl <- log.nwl.NF.org+nat.nwl.NF.org
#fwl <- log.fwl.NF.org+nat.fwl.NF.org
nwl <- log.nwl.NF.org
fwl <- log.fwl.NF.org
cwl <- log.cwl.NF.org+nat.cwl.NF.org
...
write.csv(litter.nf, (paste(resultspath,"corrected.litter.nf.",yr,".csv", sep="")))

litter.nf.awenh <- cbind(nwl, fwl, cwl)
#write.csv(litter.nf.awenh, (paste(resultspath,"litter.nf.awenh.csv", sep="")))
write.csv(litter.nf.awenh, (paste(resultspath,"corrected.litter.nf.awenh.csv", sep="")))

##calculating total litter from 1982 onwards
sum.litter.nf <- cbind(rowSums(litter.nf))
sum.litter.nf
write.csv(sum.litter.nf, (paste(resultspath,"sum.litter.nf.csv", sep="")))

##starting total litter from 1983 for calculating decomposition later as a difference between yasso result and total litter

littersum.nf <- sum.litter.nf[-1,]
littersum.nf

# second total litter for 1975-2011
#starting from 1982- because litter input data for NF starts from that year

year.nf <- seq(1982,yr, by=1)
...
write.csv(soil.nf.awenh, (paste(resultspath,"corrected.soil.nf.awenh.csv", sep="")))
###############
...
difference.nf  <-cbind(d.soil.c.nf.s)
difference.nf

log.decomp.nf <- littersum.nf-difference.nf
log.decomp.nf

write.csv(log.decomp.nf, (paste(resultspath,"log.decomp.nf.csv", sep="")))
...
write.csv(d.soil.c.nf.s.out, (paste(resultspath,"corrected.d.soil.c.tot.nf.s.mean.csv", sep="")))
