# 3.8.2020 CO2 emission factors for organic soils
# Test version 0.1
# Antti Wall & Jukka Alm, Luke
...
# path for collecting all input files to one directory for manual transfer 
# to windows G:
# Reason for transfer is to use Rstudio in windows

path2G <- '/hsan2/khk/ghg/development/organicsoils/move2G/'

#Input paths for hirsi:
...
# Output paths for hirsi:

crf <- paste(path,yr,'/crf/',sep="")
nir <- paste(path,yr,'/NIR/',sep="")

# Input paths for manual transfer to windows G:
path_on_G <- ## write here windows path for Rstudio
funpath <- path_on_G
nfidata <- path_on_G
draindata <- path_on_G
figpath <- path_on_G
resultspath <- path_on_G
undpath <- path_on_G
areas <- path_on_G

barea <- path_on_G #ppa turvekangastyypeittain vuodesta 1998 alkaen
clima <- path_on_G #touko-lokakuun keskilampotila vuodesta 1990 alkaen

# Output paths for manual transfer to windows G:
crf <- paste(path_on_G, yr, '/crf/',sep="")
nir <- paste(path_on_G, yr, '/NIR/',sep="")
...
mcagg <- aggregate(meanclima$meanT, by=list
                   (Alue=meanclima$ALUE, Vuodet=meanclima$YEAR), mean)
colnames(mcagg) <- c("Alue", "Vuodet", "meanT")


          
# Southern Finland

# UND (Aluskasvillisuus)
...
#LOG (Hakkuut?hteet nwl=non woody litt [hienojuuret ja neulaset], fwl=fine wl, cwl=coarse wl)
...
# copy input data to windows G

# climate, understorye and ppa data contain both SF and NF
# copy input data to windows G
write.csv(mcagg,file=paste(path2G,"saadata.txt", sep = ""))

# copy UND datat to windows G
write.csv(und.org.abv,file=paste(path2G,"und.org.abv.csv", sep = ""))
write.csv(und.org.bel,file=paste(path2G,"und.org.bel.csv", sep = ""))

# copy ppa data to windows G
write.csv(basal.tree, file=paste(path2G,"ppadata.txt",sep=""))

# copy all other SF data to windows G
write.csv(nwl.sf.nfi9013.org.abv,file=paste(path2G,"nwl.sf.1990",yr,".org.abv.csv", sep = ""))
write.csv(fwl.sf.nfi9013.org.abv,file=paste(path2G,"fwl.sf.1990",yr,".org.abv.csv", sep = ""))
write.csv(nwl.sf.nfi9013.org.bel,file=paste(path2G,"nwl.sf.1990",yr,".org.bel.csv", sep = ""))
write.csv(fwl.sf.nfi9013.org.bel,file=paste(path2G,"fwl.sf.1990",yr,".org.bel.csv", sep = ""))

write.csv(nat.nwl.SF.org.abv,file=paste(path2G,"nat.nwl.SF.org.abv.csv", sep=""))
write.csv(nat.nwl.SF.org.bel,file=paste(path2G,"nat.nwl.SF.org.abv.csv", sep="")
write.csv(nat.fwl.SF.org.abv,file=paste(path2G,"nat.fwl.SF.org.abv.csv", sep=""))
write.csv(nat.fwl.SF.org.bel,file=paste(path2G,"nat.fwl.SF.org.bel.csv", sep=""))
write.csv(nat.cwl.SF.org.abv,file=paste(path2G,"nat.cwl.SF.org.abv.csv", sep=""))

write.csv(log.nwl.SF.org.abv,file=paste(path2G, "log.nwl.SF.org.abv.csv", sep=""))
write.csv(log.nwl.SF.org.bel,file=paste(path2G, "log.nwl.SF.org.bel.csv", sep=""))
write.csv(log.fwl.SF.org.abv,file=paste(path2G, "log.fwl.SF.org.abv.csv", sep=""))
write.csv(log.fwl.SF.org.bel,file=paste(path2G, "log.fwl.SF.org.bel.csv", sep=""))
write.csv(log.cwl.SF.org.abv,file=paste(path2G, "log.cwl.SF.org.abv.csv", sep=""))
...
# UND
und.org.abv <-  (read.csv(paste(undpath,"und.org.abv.csv", sep="")))
und.org.bel <-  (read.csv(paste(undpath,"und.org.bel.csv", sep=""))

# TRE
nwl.nf.nfi9013.org.abv <-  read.csv(paste(nfidata,'nfi10/litter/nwl.nf.1990',yr,'.org.abv.csv', sep=""))
fwl.nf.nfi9013.org.abv <-  read.csv(paste(nfidata,'nfi10/litter/fwl.nf.1990',yr,'.org.abv.csv', sep=""))
nwl.nf.nfi9013.org.bel <-  read.csv(paste(nfidata,'nfi10/litter/nwl.nf.1990',yr,'.org.bel.csv', sep=""))
fwl.nf.nfi9013.org.bel <-  read.csv(paste(nfidata,'nfi10/litter/fwl.nf.1990',yr,'.org.bel.csv', sep=""))

#LOG
log.nwl.NF.org.abv <-  read.csv(paste(draindata,"log.nwl.NF.org.abv.csv", sep=""))
log.nwl.NF.org.bel <-  read.csv(paste(draindata,"log.nwl.NF.org.bel.csv", sep=""))
log.fwl.NF.org.abv <-  read.csv(paste(draindata,"log.fwl.NF.org.abv.csv", sep=""))
log.fwl.NF.org.bel <-  read.csv(paste(draindata,"log.fwl.NF.org.bel.csv", sep=""))
log.cwl.NF.org.abv <-  read.csv(paste(draindata,"log.cwl.NF.org.abv.csv", sep=""))

#NAT
nat.nwl.NF.org.abv <-  read.csv(paste(draindata,"nat.nwl.NF.org.abv.csv", sep=""))
nat.nwl.NF.org.bel <-  read.csv(paste(draindata,"nat.nwl.NF.org.bel.csv", sep=""))
nat.fwl.NF.org.abv <-  read.csv(paste(draindata,"nat.fwl.NF.org.abv.csv", sep=""))
nat.fwl.NF.org.bel <-  read.csv(paste(draindata,"nat.fwl.NF.org.bel.csv", sep=""))
nat.cwl.NF.org.abv <-  read.csv(paste(draindata,"nat.cwl.NF.org.abv.csv", sep=""))

# copy NF input data to windows G

write.csv(nwl.nf.nfi9013.org.abv,file=paste(path2G,"nwl.nf.1990",yr,".org.abv.csv", sep = ""))
write.csv(fwl.nf.nfi9013.org.abv,file=paste(path2G,"fwl.nf.1990",yr,".org.abv.csv", sep = ""))
write.csv(nwl.nf.nfi9013.org.bel,file=paste(path2G,"nwl.nf.1990",yr,".org.bel.csv", sep = ""))
write.csv(fwl.nf.nfi9013.org.bel,file=paste(path2G,"fwl.nf.1990",yr,".org.bel.csv", sep = ""))

write.csv(nat.nwl.NF.org.abv,file=paste(path2G,"nat.nwl.NF.org.abv.csv", sep=""))
write.csv(nat.nwl.NF.org.bel,file=paste(path2G,"nat.nwl.NF.org.abv.csv", sep=""))
write.csv(nat.fwl.NF.org.abv,file=paste(path2G,"nat.fwl.NF.org.abv.csv", sep=""))
write.csv(nat.fwl.NF.org.bel,file=paste(path2G,"nat.fwl.NF.org.bel.csv", sep=""))
write.csv(nat.cwl.NF.org.abv,file=paste(path2G,"nat.cwl.NF.org.abv.csv", sep=""))

write.csv(log.nwl.NF.org.abv,file=paste(path2G, "log.nwl.NF.org.abv.csv", sep=""))
write.csv(log.nwl.NF.org.bel,file=paste(path2G, "log.nwl.NF.org.bel.csv", sep=""))
write.csv(log.fwl.NF.org.abv,file=paste(path2G, "log.fwl.NF.org.abv.csv", sep=""))
write.csv(log.fwl.NF.org.bel,file=paste(path2G, "log.fwl.NF.org.bel.csv", sep=""))
write.csv(log.cwl.NF.org.abv,file=paste(path2G, "log.cwl.NF.org.abv.csv", sep=""))
...
#######hajotukseen lis?tty maanp??llisen karikkeen hajoaminen Ojanen 2013 mukaan
...
#######hajotukseen lis?tty maanp??llisen karikkeen hajoaminen
Rhtkg <- (475.3+41.29)*0.01
Mtkg <- (439.6+41.29)*0.01
Ptkg <- (351.8+41.29 *0.01
         Vatkg <- (291.8+41.29)*0.01
         ...
         # Copy lu.sf and lu.nf to windows G:
         write.table(lulucfareas, file=paste(path2G, "/results/", "lulucf_classes_all.txt", sep=""))
         write.table(lulucfareaskpy, file=paste(path2G, "/results/", "lulucf_rem_kptyy.txt", sep=""))
