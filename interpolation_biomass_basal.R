...
# Luetaan biomassat
bm <- read.csv(paste(PATH_input, "basal_areas_and_biomass.csv", sep = ""))
...
bm_weights <- 
  bm_melt %>%
  # Remove biomasses 8 & 9 since they are totals
  filter(tkg %in% c(2:5), fra == 1, component %ni% c("biom8", "biom9")) %>% 
